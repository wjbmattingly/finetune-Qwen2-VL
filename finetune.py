...
def train_and_validate(model_name, output_dir,dataset_name, image_column, text_column, device="cuda", user_text="Convert this image to text", num_accumulation_steps=2, eval_steps=10000, max_steps=100000, max_train=1000, max_val=100):
    ...
    train_dataset = dataset['train'].shuffle(seed=42).select(max_train)
    val_dataset = dataset['train'].shuffle(seed=42).select(max_val)
